...
class TestRAGSystemIntegration:
    ...
    def test_vector_store_data_availability(self):
        ...
        try:
            ...
            if all_metadata:
                print(
                    f"Sample metadata keys: {list(all_metadata[0].keys()) if all_metadata else 'None'}"
                )
            ...
            print(
                f"Direct search result - Documents count: {len(search_result.documents)}"
            )
            ...
            if search_result.documents:
                ...
                print(
                    f"Sample metadata: {search_result.metadata[0] if search_result.metadata else 'No metadata'}"
                )
    ...
    def test_tool_manager_execution(self):
        ...
        try:
            ...
            print(
                f"Search tool result: {search_result[:200] if search_result else 'None'}..."
            )
            ...
            print(
                f"Outline tool result: {outline_result[:200] if outline_result else 'None'}..."
            )
    ...
    def test_session_management(self):
        ...
        try:
            ...
            self.rag_system.session_manager.add_exchange(
                test_session, "What is MCP?", "MCP stands for Model Context Protocol"
            )
            ...
            formatted = self.rag_system.session_manager.get_formatted_history(
                test_session
            )
